...
public final class ClientLockOnEvents {
    ...
    public static void renderCursor(RenderLivingEvent.Post<LivingEntity, ?> event) {
        ...
        poseStack.pushPose();
        poseStack.translate(0.0D, event.getEntity().getBbHeight() * 0.65D, 0.0D);
        poseStack.mulPose(Minecraft.getInstance().getEntityRenderDispatcher().cameraOrientation());
        poseStack.mulPose(Axis.YP.rotationDegrees(180.0F));
        poseStack.scale(0.8F, 0.8F, 0.8F);

        VertexConsumer consumer = event.getMultiBufferSource().getBuffer(RenderType.entityTranslucent(CURSOR));
